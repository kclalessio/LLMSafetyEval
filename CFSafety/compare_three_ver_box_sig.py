...
versions   = ['base', 'PE', 'sft-en']
versions_to_text = {'base': 'Base','PE': 'Prompt Eng','sft-en': 'SFT (English)'}
categories = [
...
output_dir = 'plots/3_version_comparison_with_box_signif'
...
for lang in languages:
    ...
    ax.set_title(f'Base, Prompt Eng and SFT (English) Comparison in {languages_to_text[lang]} for All Models', fontsize=14)
    ax.set_xticks(x)
        ...
        for v1, v2 in comparisons:
            ...
            for lbl_idx, lbl in enumerate(labels):
                ...
                ax.text((x1 + x2p)/2, y+0.05, stars,
                        ha='center', va='top', fontsize=9,
                        bbox=dict(boxstyle="round,pad=0.05", facecolor="white", edgecolor="none"), zorder=6)


        ax.set_title(f'Base, Prompt Eng and SFT (English) Comparison in {languages_to_text[lang]} for Model: {models_to_text[model]}', fontsize=14)
        label_texts = ['Overall'] + list(categories_to_text.values())
        tick_pos = [
            np.mean([ bar_positions[v][idx] for v in versions ])
